...
class infill_generator
{
    ...
    std::array<section, 2> create_generator_sections(const plane &, double h, double angle) const;
    ...
    section generate_relevant_section(const plane &, const section &intersector) const;

public:
    infill_generator(frep_object *);

    polylines generate(const plane &, double h, double angle, double space_between) const;
};
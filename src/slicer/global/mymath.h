...
inline mat4 operator*(const mat4 &left, const mat4 &right)
{
	mat4 result;
	for (int i = 0; i < 4; i++)
		result.rows[i] = left.rows[i] * right;
	return result;
}
...
inline mat4 RotationMatrix(double angle, vec3 w)
{
	...
	m[0][0] = c * (1 - w.x * w.x) + w.x * w.x;
	m[0][1] = w.x * w.y * (1 - c) - w.z * s;
	m[0][2] = w.x * w.z * (1 - c) + w.y * s;
	m[0][3] = 0;
	m[1][0] = w.x * w.y * (1 - c) + w.z * s;
	m[1][1] = c * (1 - w.y * w.y) + w.y * w.y;
	m[1][2] = w.y * w.z * (1 - c) - w.x * s;
	m[1][3] = 0;
	m[2][0] = w.x * w.z * (1 - c) - w.y * s;
	m[2][1] = w.y * w.z * (1 - c) + w.x * s;
	m[2][2] = c * (1 - w.z * w.z) + w.z * w.z;
	m[2][3] = 0;
	m[3][0] = 0;
	m[3][1] = 0;
	m[3][2] = 0;
...
inline double sin_d(double val, const unsigned int n)
{
	switch (n % 4)
	{
	case 0:
...
inline double cos_d(double val, const unsigned int n)
{
	switch (n % 4)
	{
	case 0:

...
struct vec3_t
{
    ...
    vec3_t operator*(double num) const { return vec3_t(x * num, y * num, z * num); }
    vec3_t operator/(double num) const { return vec3_t(x / num, y / num, z / num); }
...
template <typename T>
inline double dot(const vec3_t<T> &v1, const vec3_t<T> &v2) { return (v1.x * v2.x + v1.y * v2.y + v1.z * v2.z); }
